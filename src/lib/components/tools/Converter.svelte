<script lang="ts">
  ...
  function setCategory(cat: Category) {
    ...
    toUnit = units.find(u => u.name.includes('Feet') || u.name.includes('Pounds') || u.name.includes('Gallons') || u.name.includes('Fahrenheit'))?.id || units[1].id; // Try to find an imperial default
  }
...
<div class="h-full flex flex-col items-center justify-center p-8 bg-base-100/50">
  <div class="card w-full max-w-2xl bg-base-100 shadow-xl border border-base-200">
    <div class="card-body p-0">
      
      <!-- Category Tabs (Header) -->
      <div class="flex flex-wrap border-b border-base-200 bg-base-200/30 rounded-t-xl">
        {#each Object.entries(categories) as [key, cat]}
          <button 
            class="flex-1 py-4 flex flex-col items-center gap-1 transition-all hover:bg-base-100 
                   {activeCategory === key ? 'bg-base-100 text-primary border-b-2 border-primary -mb-[2px] font-bold' : 'text-base-content/60'}"
      ...
      <div class="p-8 grid gap-8">
        ...
        <div class="relative group">
          <label class="label text-xs uppercase font-bold text-base-content/50 mb-1" for="input-val">
            Convert from
          </label>
          <div class="flex items-center gap-4">
            <input 
              ...
              placeholder="0"
            />
            <select bind:value={fromUnit} class="select select-bordered select-lg max-w-xs font-medium">
              {#each currentUnits as unit}
                <option value={unit.id}>{unit.name}</option>
              {/each}
            </select>
          </div>
          <div class="h-[1px] bg-base-200 w-full mt-2 group-focus-within:bg-primary transition-colors"></div>
        </div>
        ...
        <div class="flex items-center justify-center -my-4 z-10">
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M7 10h14l-4-4"/><path d="M17 14H3l4 4"/></svg>
          </button>
        </div>

        <!-- To Section -->
        <div class="relative">
          <label class="label text-xs uppercase font-bold text-base-content/50 mb-1" for="output-val">
            To
          </label>
          <div class="flex items-center gap-4 opacity-100">
            <div class="text-4xl font-bold text-primary w-full truncate" title={result.toString()}>
              {format(result)}
            </div>
            <select bind:value={toUnit} class="select select-bordered select-lg max-w-xs font-medium">
              {#each currentUnits as unit}
                <option value={unit.id}>{unit.name}</option>
              {/each}
            </select>
      ...
      <div class="bg-base-200/50 p-4 text-center text-sm text-base-content/60 rounded-b-xl">
        1 {fromUnitObj?.name} = {format(result/createInput)} {toUnitObj?.name}
      </div>
