...
export async function GET() {
    ...
    try {
        ...
        function extractColors(themeName: string): Record<string, string> {
            const regex = new RegExp(`name:\\s*"${themeName}"[\\s\\S]*?([\\s\\S]*?)\\}`, 'gm');
        ...
        // Look for the block containing name: "saasstartertheme" and check if default: true is before the closing brace
        function isBlockDefault(themeName: string): boolean {
        ...
        const isDarkDefault = isBlockDefault('saasstartertheme');
