...
export async function POST({ request }) {
    ...
    try {
        ...
        function updateThemeBlock(content: string, themeName: string, newColors: Record<string, string>, isDefault: boolean): string {
            ...
            // Clean up newlines if we added them messily
            // (Optional, mostly for readability)
        ...
        // Apply updates
        if (colors.dark) {
            cssContent = updateThemeBlock(cssContent, 'saasstartertheme', colors.dark, defaultTheme === 'dark');
        }
        if (colors.light) {
            cssContent = updateThemeBlock(cssContent, 'saasstartertheme-light', colors.light, defaultTheme === 'light');
        }
