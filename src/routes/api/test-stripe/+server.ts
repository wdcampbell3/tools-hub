...
export const GET: RequestHandler = async () => {
  try {
    ...
    // Determine key type
    const keyType = stripeApiKey.startsWith('sk_live_') ? 'live' :
                    stripeApiKey.startsWith('sk_test_') ? 'test' :
                    stripeApiKey.startsWith('rk_live_') ? 'restricted_live' :
                    stripeApiKey.startsWith('rk_test_') ? 'restricted_test' : 'unknown';
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (action === 'list-products') {
      // List products
      const response = await fetch('https://api.stripe.com/v1/products?limit=10', {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${stripeApiKey}`
        }
    ...
    if (action === 'list-prices') {
      // List prices
      const response = await fetch('https://api.stripe.com/v1/prices?limit=10', {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${stripeApiKey}`
        }
