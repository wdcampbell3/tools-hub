<script lang="ts">
    ...
    const colorDefs = [
        { key: "--color-primary", label: "Primary", description: "Brand Action, Links" },
        { key: "--color-secondary", label: "Secondary", description: "Decorators, Alt Buttons" },
        { key: "--color-accent", label: "Accent", description: "Highlights, Focus Rings" },
        { key: "--color-neutral", label: "Neutral", description: "Dark Surfaces, Footers" },
        
        { key: "--color-base-content", label: "Base Text", description: "Main Body Text" },
        { key: "--color-base-100", label: "Base 100", description: "Main Background" },
        { key: "--color-base-200", label: "Base 200", description: "Cards, Inputs, Sidebar" },
        { key: "--color-base-300", label: "Base 300", description: "Hover States, Borders" },

        { key: "--color-info", label: "Info", description: "Alerts, Badges" },
        { key: "--color-success", label: "Success", description: "Completion, Success" },
        { key: "--color-warning", label: "Warning", description: "Caution, Attention" },
        { key: "--color-error", label: "Error", description: "Destructive, Failures" },
    ];
    ...
    function restoreDefaults() {
        if (confirm("Are you sure you want to restore the default theme colors? This will overwrite your current unsaved changes.")) {
             lightColors = { ...DEFAULT_THEME.light };
             darkColors = { ...DEFAULT_THEME.dark };
             applyTheme(isLight);
             alert("Theme restored to defaults. Remember to 'Save' to make it permanent.");
        }
    }
...
<div class="container mx-auto p-10 space-y-24 relative">
    ...
    <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center border-b border-base-content/10 pb-10 gap-6">
        <div>
           <h1 class="text-4xl font-bold mb-2">Design System</h1>
           <p class="text-base-content/70 text-lg">A unified reference for UI elements, colors, and typography.</p>
        </div>

        <div class="flex flex-col md:flex-row items-end md:items-center gap-6 w-full xl:w-auto">
             ...
             <div class="flex items-center gap-2">
                ...
                {#if saveMessage}
                    <span class="text-xs text-success animate-fade-in px-2">{saveMessage}</span>
                {/if}
            </div>
            ...
            <div class="flex flex-col items-end gap-3">
                <!-- Theme Toggle -->
                <div class="flex items-center gap-3 bg-base-200 p-2 rounded-full border border-base-content/10">
                    <span class="text-sm font-medium px-2 {isLight ? 'opacity-50' : 'text-primary'}">Dark</span>
                    <input type="checkbox" class="toggle toggle-primary" checked={isLight} onchange={toggleTheme} />
    ...
    <!-- Typography -->
    <section>
        <h2 class="text-xl font-bold border-b border-base-content/10 pb-4 mb-8 uppercase tracking-wider opacity-70">Typography</h2>
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <div class="space-y-6">
                <div class="flex items-baseline justify-between border-b border-base-content/5 pb-2">
    ...
    <section>
        <h2 class="text-xl font-bold border-b border-base-content/10 pb-4 mb-8 uppercase tracking-wider opacity-70">Color Palette (Interactive)</h2>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
            {#each colorDefs as color}
                <div class="card p-4 shadow-sm flex flex-col gap-3 relative group">
                    <!-- Color Swatch & Picker -->
                    <div 
                        class="w-full h-24 rounded-lg shadow-inner cursor-pointer relative overflow-hidden transition-transform hover:scale-[1.02]" 
                    ...
                    <div class="flex flex-col">
                        ...
                        <span class="text-xs opacity-60 uppercase font-semibold">{color.description}</span>
                    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-16">
        <!-- Buttons -->
        <section>
            <h2 class="text-xl font-bold border-b border-base-content/10 pb-4 mb-8 uppercase tracking-wider opacity-70">Interactive</h2>
            <div class="space-y-8">
                 <div class="space-y-2">
                ...
                <div class="space-y-2">
                     ...
                     <div class="flex flex-wrap gap-3">
                        ...
                        <button class="btn btn-primary" disabled>Disabled</button>
                         <button class="btn btn-primary"><span class="loading loading-spinner loading-xs"></span> Loading</button>
                    </div>
                </div>
                 ...
                 <div class="space-y-2">
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        <button class="btn btn-lg bg-base-200 border border-base-content/10 hover:bg-base-300">Large</button>
                        <button class="btn bg-base-200 border border-base-content/10 hover:bg-base-300">Normal</button>
                        <button class="btn btn-sm bg-base-200 border border-base-content/10 hover:bg-base-300">Small</button>
                        <button class="btn btn-xs bg-base-200 border border-base-content/10 hover:bg-base-300">Tiny</button>
                    </div>
                </div>
            </div>
        </section>
        ...
        <!-- Inputs -->
        <section>
            <h2 class="text-xl font-bold border-b border-base-content/10 pb-4 mb-8 uppercase tracking-wider opacity-70">Inputs & Forms</h2>
            <div class="grid grid-cols-1 gap-6">
                <div class="form-control w-full">
                    <label class="label"><span class="label-text font-medium">Text Input</span></label>
                    <input type="text" placeholder="Type here..." class="input input-bordered w-full" />
                </div>
                 <div class="grid grid-cols-2 gap-4">
                    <div class="form-control w-full">
                        <label class="label"><span class="label-text font-medium">Select</span></label>
                    ...
                    <div class="form-control w-full">
                        <label class="label"><span class="label-text font-medium">File Input</span></label>
                        <input type="file" class="file-input file-input-bordered w-full" />
                    </div>
                 </div>

                 <div class="flex flex-wrap gap-8 p-4 bg-base-200 rounded-box border border-base-content/10">
                    ...
                    <div class="form-control">
                        <label class="label cursor-pointer justify-start gap-3">
                            <input type="radio" name="radio-preview" class="radio radio-primary" checked />
     ...
     <!-- UI Elements -->
    <section>
        <h2 class="text-xl font-bold border-b border-base-content/10 pb-4 mb-8 uppercase tracking-wider opacity-70">Feedback & Cards</h2>
             ...
             <div class="flex flex-col gap-8">
                ...
                <div class="card bg-base-100 shadow-xl">
                    <div class="card-body">
                        ...
                        <p class="text-base-content/70">A standard card container for grouping related content and actions.</p>
